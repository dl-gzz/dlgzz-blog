...
/**
 * OneWorkOS managed installer. Compatible with Node.js 18/20/22 both as a
 * downloaded .mjs file and through `node -` (no static ESM imports).
 *
 * Preferred:
 *   curl -fsSL https://www.dlgzz.com/downloads/onework-install.mjs -o /tmp/onework-install.mjs
 *   node /tmp/onework-install.mjs --server https://www.dlgzz.com --token "owinst_..."
 *
 * Credentials:
...
async function main() {
  ...
  function validateRelease(value) {
    if (
      value?.schema !== 'onework.skill-release.v1' ||
      value?.name !== 'one-work-os' ||
  ...
  if (serverUrl.protocol === 'http:' && !isLocalhost(serverUrl))
    throw new Error('远程 OneWorkOS 安装必须使用 HTTPS');
  ...
  if (!/^[A-Za-z0-9._:-]{16,200}$/.test(deviceId)) {
    throw new Error('OneWorkOS 设备标识无效');
  }
  ...
  if (!skipSkill) {
    ...
    skillSource = join(extractedRoot, 'one-work-os');
  ...
  if (!/^[A-Za-z0-9._~-]{20,500}$/.test(claimData.key.rawKey)) {
    throw new Error('安装服务器返回了无效的 OneWorkOS Key');
  }
  ...
  const credentialBackup = join(backupSession, 'one-work-os.local.env');
  const skillBackup = join(backupSession, 'one-work-os');
  ...
  if (!verifyResponse.ok || !verifyData.success) {
    throw new Error(
      `OneWorkOS 文件已安装，但设备授权验证失败（HTTP ${verifyResponse.status}）：${verifyData.error || '请重启 WorkBuddy 后重试'}`
    );
  }
  ...
  console.log(`OneWorkOS 已连接：${claimData.key.keyPrefix}`);
