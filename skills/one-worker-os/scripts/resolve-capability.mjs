...
} from './one-worker-os-credentials.mjs';
...
async function fetchWithTimeout(url, init, timeoutMs = 15_000) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new Error(
        `one-worker-os 能力路由超时（${Math.round(timeoutMs / 1000)} 秒），请检查网络后重试。`
      );
    }
    throw error;
  } finally {
...
async function main() {
  ...
  if (!response.ok) {
    if (data?.nonJson) {
      ...
      throw new Error(
        `one-worker-os API HTTP ${response.status}: resolver returned ${contentType}; check deployment of /api/capabilities/resolve`
      );
    }
  ...
  if (!data?.success || !data.resolution) {
    throw new Error('one-worker-os API returned an invalid resolver response');
  }
