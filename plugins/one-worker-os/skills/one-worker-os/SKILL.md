...
Different OAuth clients can remain connected to the same member account. Reauthorization replaces only that client's previous token family after token exchange succeeds; a denial or failed exchange must not disconnect a working connection. Client identity is the OAuth client ID, not a physical-computer count. Disconnecting one client does not disconnect other clients, sign the user out of the website, cancel membership, or delete knowledge.

When asked how many computers can be connected, read `authorizationPolicy` from `onework_get_entitlements`. The current policy has no fixed account-wide active-connection count; independently registered clients keep their own connections and share the account's membership and usage. Do not promise that two installations sharing the same client ID are independent. Use the live policy if it differs from this package.
