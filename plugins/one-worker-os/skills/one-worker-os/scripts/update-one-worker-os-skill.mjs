...
const stateFile = join(workbuddyRoot, 'one-worker-os-update-state.json');
const backupRoot = join(workbuddyRoot, 'one-worker-os-backups');
...
function readVersion(root) {
  ...
  if (!version || !/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(version)) {
    throw new Error('one-worker-os manifest.yaml 缺少有效版本号');
  }
  return version;
}
...
function releaseUrl() {
  ...
  if (
    ...
    throw new Error('远程 one-worker-os 更新必须使用 HTTPS');
  }
...
function validateRelease(release) {
  if (
    release?.schema !== 'one-worker-os.skill-release.v1' ||
    release?.name !== 'one-worker-os' ||
...
function cleanupBackups(keep = 2) {
  ...
  const backups = readdirSync(backupRoot, { withFileTypes: true })
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith('one-worker-os-')
    )
    .map((entry) => ({
      name: entry.name,
...
async function installUpdate(release, manifestUrl) {
  ...
  const stagingRoot = mkdtempSync(
    join(workbuddyRoot, '.one-worker-os-update-staging-')
  );
  ...
  try {
    ...
    const stagedSkill = join(stagedFiles, 'one-worker-os');
    validateExtractedSkill(stagedSkill, release);
    mkdirSync(backupRoot, { recursive: true, mode: 0o700 });
    const backupPath = join(
      backupRoot,
      `one-worker-os-${readVersion(skillRoot)}-${Date.now()}`
    );
...
main()
  .then((result) => {
    if (process.argv.includes('--json'))
      ...
      console.log(
        `one-worker-os 已更新到 ${result.currentVersion}，请重启 WorkBuddy。`
      );
    else console.log(`one-worker-os ${result.currentVersion} 已是当前版本。`);
  })
  .catch((error) => {
