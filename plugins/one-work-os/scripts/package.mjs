...
const canonicalSkill = join(projectRoot, 'skills', 'one-work-os');
const pluginSkill = join(pluginRoot, 'skills', 'one-work-os');
const marketplacePath = join(
  projectRoot,
  '.codebuddy-plugin',
  'marketplace.json'
);
const outputRoot = join(projectRoot, 'public', 'onework-marketplace');
...
function validateSource() {
  ...
  if (plugin.name !== 'one-work-os') {
    throw new Error('plugin.json name must be one-work-os');
  }
  ...
  if (marketplace.name !== 'onework-os-marketplace') {
    throw new Error('marketplace name must be onework-os-marketplace');
  }
  ...
  if (
    ...
    throw new Error(
      'marketplace must contain the local OneWorkOS plugin source'
    );
  }

  const server = mcp.mcpServers?.['onework-os'];
  if (
...
function buildOnce(version) {
  const temporaryRoot = mkdtempSync(join(tmpdir(), 'onework-plugin-package-'));
  try {
    const pluginStage = join(temporaryRoot, 'one-work-os');
    copyPlugin(pluginStage);
    const pluginZipPath = join(
      temporaryRoot,
      `one-work-os-plugin-${version}.zip`
    );
    ...
    const marketplaceStage = join(temporaryRoot, 'onework-marketplace');
    ...
    copyPlugin(join(marketplaceStage, 'plugins', 'one-work-os'));
    const marketplaceZipPath = join(
      temporaryRoot,
      `onework-os-marketplace-${version}.zip`
    );
    ...
    return {
      plugin: { ...pluginArchive, name: `one-work-os-plugin-${version}.zip` },
      marketplace: {
        ...marketplaceArchive,
        name: `onework-os-marketplace-${version}.zip`,
      },
    };
  } finally {
...
function main() {
  ...
  const release = {
    schema: 'onework.plugin-release.v1',
    name: plugin.name,
