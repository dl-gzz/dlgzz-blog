...
- Treat OneWorkOS as the control plane: resolve intent, select capabilities, order work, enforce policy, and verify completion.
...
Keep registry state on the OneWorkOS service. Keep provider credentials and runtime tool grants in the user's host environment.

## Failure handling

- On `401`, request valid OneWorkOS credentials without asking the user to paste the secret into chat.
