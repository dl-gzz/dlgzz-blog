...
export async function GET(req: NextRequest) {
  try {
    ...
    const requirePremium =
      searchParams.get('premium') === 'true' || access === 'premium';
    ...
    try {
      ...
      // 统计记录失败不影响下载
      console.warn(
        '⚠️ Failed to record download stats (database not configured):',
    ...
    // 控制台日志
    console.log(
      `File downloaded: ${fileName} by user: ${userEmail || 'anonymous'}`
    );
