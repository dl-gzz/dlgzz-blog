...
const TOOLS = [
  {
    ...
    description:
      'Resolve a goal to governed OneWorkerOS capabilities and an execution route.',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description:
      'Search a licensed OneWorkerOS knowledge pack using the governed retrieval service.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['query'],
      properties: {
        ...
        packId: {
          ...
          maxLength: 160,
          description:
            'Optional licensed pack ID. Omit or pass auto so OneWorkerOS routes WorkBuddy and Xiaohongshu automatically.',
        },
        context: {
          type: 'string',
          maxLength: 1000,
  ...
  },
  {
    ...
    description:
      'Validate or execute a governed OneWorkerOS semantic analytics query.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
  ...
  },
  {
    ...
    description: 'Get current-month OneWorkerOS usage and remaining quota.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {},
...
function ensureQuota(access: Awaited<ReturnType<typeof listOneWorkAccess>>) {
  if (access.usage.limit < 1 || access.usage.remaining < 1) {
    throw new McpToolError(
      'QUOTA_EXCEEDED',
      'Monthly OneWorkerOS quota exhausted'
    );
  }
}
    ...
    if (request.method === 'initialize') {
      ...
      return {
        response: rpcSuccess(id, {
          ...
          serverInfo: { name: 'OneWorkerOS', version: '1.0.0' },
          instructions:
            'Use OneWorkerOS tools only within the OAuth scopes granted to this connection.',
        }),
        status: 200,
      };
    }
          ...
          response: rpcSuccess(
            ...
            asToolError(
              new McpToolError(
                ...
                'The OneWorkerOS tool could not complete the request'
              )
            )
          ),
          status: 200,
