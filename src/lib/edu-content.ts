...
async function ensureUniqueSlug(seed: string, locale: string) {
  ...
  for (let index = 0; index < 1000; index += 1) {
    ...
    const exists =
      filePostExists(candidate, locale) ||
      (await databasePostExists(candidate, locale));
...
function rowToCoursewarePost({
  ...
  const htmlContent =
    courseware?.htmlContent || extractSavedCoursewareHtml(body);
...
export async function savePromptBlockToDatabase({
  ...
  const safeTitle = readText(
    title,
    readText(frontmatter.title, 'AI 课件提示词 Block')
  );
  const safeDescription =
