...
When you use one-worker-os knowledge retrieval, your query text may be sent to Zhipu AI to generate a temporary embedding needed for semantic search; results and necessary request metadata are sent to the one-worker-os service. Only specifically authorized articles are chunked, embedded, and written to the production knowledge base. We do not ask you to provide or display an API key, device ID, or access token. Please do not include unnecessary sensitive personal information in your prompts.
