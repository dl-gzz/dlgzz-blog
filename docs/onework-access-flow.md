...
1. 管理员打开登录后的 `/admin/onework`，选择知识包，生成一次性 `OWOS-...` 兑换码。
2. 把兑换码发给购买用户。
...
ONEWORK_PACK_IDS=onework-workbuddy-v1,xhs-open-shop-v1
```

支付首次从 `processing` 变成 `completed` 时，系统直接给账号写入权益；重复回调不会重复延长同一笔订单。
...
2. 配置 `ONEWORK_PRICE_IDS` / `ONEWORK_PACK_IDS`（如果只卖外部平台兑换码，可以留空）。
