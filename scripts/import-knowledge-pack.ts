...
function listMarkdownFiles(dir: string, recursive = false): string[] {
	...
	for (const entry of readdirSync(dir).sort((a, b) => a.localeCompare(b, "zh-Hans-CN"))) {
			...
			for (const filePath of listMarkdownFiles(dirPath, entry.recursive)) {
				...
				docs.push({
					...
					metadata: {
						...
						corpus: source,
						relativePath: relative(manifest.packDir, filePath),
					},
				});
			}
			continue;
...
function prepareDoc(doc: SourceDoc): PreparedDoc {
	...
	const sourceUrl = readString((parsed.data as Record<string, unknown>).source_url);
	const metadata = sourceUrl
...
async function main() {
	...
	for (const rule of manifest.units) {
		...
		if (rule.dir) {
			for (const filePath of listMarkdownFiles(join(manifest.packDir, rule.dir))) {
	...
	if (options.dryRun) {
		...
		for (const doc of docs) {
			...
			console.log(
				`- [${doc.source}] ${doc.title} (${relative(manifest.packDir, doc.filePath)})${isUnitFile ? " [heading_qa units]" : ""}`
			);
		}
		return;
	}
	...
	try {
		...
		for (const doc of docs) {
			try {
				...
				if (unitRule) {
					const units = extractHeadingQaUnits(doc, manifest, unitRule.riskLevel || "low");
