/** Apply the additive OneWorkOS capability and semantic-layer migration. */
...
async function main() {
  ...
  try {
    ...
    `;
    if (rows.length !== 4) {
      throw new Error('OneWorkOS semantic-layer tables were not created');
    }
    console.log(
      `OneWorkOS semantic layer ready: ${rows
