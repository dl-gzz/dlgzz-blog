...
function getInstanceId() {
  const id = process.argv
    .slice(2)
    .find((arg) => arg.trim() && arg !== '--')
    ?.trim();
  if (!id)
    throw new Error('Usage: pnpm worker:provision -- <workerInstanceId>');
  return id;
}
...
function getBridgeHeaders(): Record<string, string> {
  ...
  return token
    ? { Authorization: `Bearer ${token}`, 'X-Hermes-Bridge-Token': token }
    : {};
}
...
async function main() {
  ...
  try {
    ...
    const knowledgePacks = [
      ...employeeKnowledgePacks,
      ...skillKnowledgePacks,
    ].filter(
      (pack, index, all) =>
        all.findIndex((item) => item.id === pack.id) === index
    );
    ...
    const response = await fetch(
      new URL('/assistants/provision', getBridgeUrl()),
      {
        method: 'POST',
        headers: {
          ...getBridgeHeaders(),
    ...
    if (!response.ok || !data?.success) {
      throw new Error(
        data?.error || `Hermes Bridge failed: ${response.status}`
      );
    }
    ...
    await sql`
			update worker_instance set
				...
				activated_at = ${data.status === 'activated' ? new Date() : null},
				updated_at = now()
