...
function main() {
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), 'onework-skill-package-'));
  const stagedSkill = join(temporaryRoot, skillName);
  const temporaryZip = join(
    temporaryRoot,
    `oneworkos-workbuddy-skill-${manifest.version}.zip`
  );
  const versionedFilename = basename(temporaryZip);
  const latestFilename = 'oneworkos-workbuddy-skill-latest.zip';
  ...
  try {
    ...
    const release = {
      schema: 'onework.skill-release.v1',
      name: manifest.name,
